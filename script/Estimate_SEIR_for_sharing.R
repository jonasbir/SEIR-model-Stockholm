...
# R version:    3.5.2
#
# What the script does: It estimates the parameters of the infectivity described in the report 
...
# input your own path where folder is located in project.path.
project.path 	     <- ""
...
Stockholm_Data_10_april <- read.table(file=paste(data.path,"/Data_2020-04-14.txt",sep=""), sep = " ", header=TRUE)
...
## For faster analysis use different tolerance, but know what it could mean.
Atol = 1e-8
Rtol = 1e-10
...
# Analysis  p_symp_use <- 0.01 , 0.005, 0.001
# Analysis  p_lower_inf_use <- 1, 0.55, 0.11

# Sensitivity  p_symp_use <- 0.05 , 0.5, 0.9
# Sensitivity  p_lower_inf_use <- 1, 0.5, 0.1
...
# Days of incidence based on region
...
Est
Opt_par <- setNames(Est$par, c("p","epsilon","theta"))
Opt_par
...
fitted_incidence_non_reported  <- p_lower_inf_use* beta(fit[,1], delta = Opt_par[1], epsilon = Opt_par[2], theta = Opt_par[3]) * fit_S *  fit_I_asymp/N 
...
plot(c(0:150),Basic_repr(c(0:150), p = Est$par[1], epsilon = Est$par[2],  theta = Est$par[3]  ,gamma = gammaD),type="l", ylab="R0(t)",lwd=2, 
...
p_high        <- qnorm(1-CI_level_05, mean = Opt_par[1], sd = sdParams[1], lower.tail = TRUE, log.p = FALSE)
...
p_low         <- max(0,qnorm(CI_level_05, mean = Opt_par[1], sd = sdParams[1], lower.tail = TRUE, log.p = FALSE))
...
p.v        <- rnorm(1000, mean = Opt_par[1], sd = sdParams[1])
p.v[which(p.v<0)] <- 0
...
for(i in 1:length(p.v)){
  ...
  R0.v.Dag1[i]      <- Basic_repr(Day[1],p = p.v[i], epsilon = epsilon.v[i], theta = theta.v[i], gamma = gammaD) 
  R0.v.DagSista[i]  <- Basic_repr(Day[length(Day)],p = p.v[i], epsilon = epsilon.v[i], theta = theta.v[i], gamma = gammaD) 
}
...
R0_Mean <- Basic_repr(Day, p = Est$par[1], epsilon = Est$par[2],  theta = Est$par[3]  ,gamma = gammaD)
...
fitted_incidence        <- beta(fit[,1], delta = Opt_par[1], epsilon = Opt_par[2], theta = Opt_par[3]) * fit_S *  fit_I_symp / N 
fitted_incidence_asymp  <- p_lower_inf_use * beta(fit[,1], delta = Opt_par[1], epsilon = Opt_par[2], theta = Opt_par[3]) * fit_S *  fit_I_asymp/N 
...
for(i in 1:length(p.v)){
  Opt_parDummy = c(p = p.v[i], epsilon = epsilon.v[i], theta = theta.v[i])
